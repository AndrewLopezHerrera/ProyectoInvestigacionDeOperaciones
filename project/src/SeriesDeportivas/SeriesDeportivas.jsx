...
export default function SeriesDeportivas() {
  ...
  const calcularProbabilidadNormal = useCallback(() => {
    ...
    descargarJSON("serie_normal.json", datosGenerados);
    // -----------------------------
  ...
  const calcularProbabilidadLocalia = useCallback(() => {
    ...
    // Formato usando "ph" o "pv"
    const formatoEtiquetas = formatoLocalia.map((esLocalA) =>
      esLocalA ? "ph" : "pv"
    );
    ...
    descargarJSON("serie_localia.json", datosGenerados);

    // Cálculo de probabilidad con recursión
    const kGanador = Math.floor(numJuegos / 2) + 1;
    const memo = new Map();
    ...
    function resolver(juegosGanadosA, juegosGanadosB) {
      ...
      const esLocalA = formatoLocalia[numeroJuego]; // true=ph, false=pv
  ...
  const reiniciarModo = () => {
    ...
    setTimeout(() => {
      ...
      setProbCasa(0.6);
      setProbVisita(0.4);
      setFormatoLocalia(new Array(7).fill(true));
      ...
      try {
        ...
        if (modo === "localia") {
          if (
            ...
            setError("El archivo debe incluir valores válidos para 'ph' y 'pv' (entre 0 y 1).");
            return;
          }

          // Validar formato si existe
          if (datos.formato) {
            if (!Array.isArray(datos.formato)) {
              setError("El campo 'formato' debe ser un arreglo.");
              return;
            }

            if (datos.formato.length !== datos.n) {
              setError("El arreglo 'formato' debe tener exactamente n elementos.");
              return;
            }

            // Convertir "ph" o "pv" a booleanos (true = A local, false = B local)
            const nuevoFormato = datos.formato.map((v) => {
              if (v === "ph") return true;
              if (v === "pv") return false;

              setError("Los valores de 'formato' deben ser 'ph' o 'pv'.");
              throw new Error("Formato inválido");
            });

            setFormatoLocalia(nuevoFormato);
          }
